...
1. Site has global reach — "residents" framing is too state-bounded; reword to "users."
2. The backend migration described as future / planned in earlier revisions is in fact complete; managers reading the exec summary should see the right status, not the work plan from before the migration ran.
3. Remove the embedded UI render from the README — leadership shouldn't open the planning repo and see a draft front-end as if it were a settled artifact.
4. Point all migration references at [`ICJIA/icjia-migration-tools`](https://github.com/ICJIA/icjia-migration-tools) so managers can verify what was delivered.
5. Surface that the Research Hub is leaving the public-site bundle, document the SEO-equity preservation strategy for `/researchhub/...` URLs, and explain the implications for the public-site rebuild's scope.
6. Document why the Strapi 3 → 5 migration was necessary for **both** the public site and the Hub, what Strapi 5 brings that Strapi 3 did not, and the accessibility benefits — both for staff editors using the admin and for content as it reaches the public.

### Added

- **`docs/01-EXECUTIVE-SUMMARY.md` §1.1 — "What's in scope, what's separated out: the Research Hub."** New subsection (~5 paragraphs) under §1 explaining: today's bundled site at `icjia.illinois.gov` is two things (the public site + the Research Hub), the Hub accounts for ~65–80% of traffic, the Hub is being separated into its own project on its own infrastructure with completion expected within months, hub URLs are preserved exactly for SEO equity, and the public-site rebuild is therefore considerably lighter than a like-for-like rebuild would have been. Honest framing of the in-flux nature of the structural change with an explicit statement that the SEO outcome is settled.
- **`docs/01-EXECUTIVE-SUMMARY.md` §3 — Strapi 5 benefits + accessibility benefits + why-both rationale.** Three new paragraphs under the "publishing tool was past end-of-life — and has now been replaced" reason: (a) what Strapi 5 brings that Strapi 3 did not (continued vendor support, native author preview, modern accessible admin, explicit draft/publish state, granular per-content-type permissions); (b) accessibility benefits specifically (admin UI accessibility for staff editors with disabilities, preview catches a11y issues before publish, EOL CMS is itself a compliance finding closed by the move); (c) why migrating both the main site and the Hub was necessary, not optional (both ran v3, EOL applied to both, operating two CMS versions afterward would have forked agency operations and left the majority of agency traffic on unsupported software).
- **`docs/01-EXECUTIVE-SUMMARY.md` §6 lead — "Two pieces … already accounted for."** Replaces the single "one half of the work is already done" line with a two-bullet inventory: (a) the publishing-tool migration is complete with link to `icjia-migration-tools`; (b) the Hub is leaving the bundle per §1.1. Sets the scope for the rest of §6 explicitly.
- **`docs/01-EXECUTIVE-SUMMARY.md` §7 timeline — "Already accounted for, out of scope" callout.** Two-bullet list noting that the publishing-tool upgrade and the Hub project are both outside the 6–8-week front-end estimate.
- **README.md — backend-migration status banner.** "The backend Strapi 3 → Strapi 5 migration is **complete**" with link to `icjia-migration-tools` as evidence for managers visiting the planning repo.
- **`docs/00-README.md` — backend-migration status paragraph.** Documents the completed backend move at the top of the planning-set index so engineers reading the index see the current state immediately.
- **`docs/02-MASTER-DESIGN-PLAN.md` §1.2 — backend-status callout + scope callout.** Two block-quote notes: backend migration is complete (links to `icjia-migration-tools` and the runbook); content split table is for the public-site portion only (Hub content types are out of scope and owned by the Hub project). Hub line in the content split table is struck through with a "separate project" annotation.
- **`docs/02-MASTER-DESIGN-PLAN.md` §3.2 — `researchhub/` URL clarification.** Reframes the URL convention as cross-project canonical-URL preservation: hub URLs continue to resolve identically (search engines, bookmarks, partner-agency links unaffected), routed at the edge or via the Nuxt redirect table to the dedicated Hub project's infrastructure. SEO-equity rationale stated explicitly.

### Changed

- **`docs/01-EXECUTIVE-SUMMARY.md` §1 short version — past-tense backend, link to migration tool.** "Behind the scenes, the publishing tool … *has already been moved*" with link to [`ICJIA/icjia-migration-tools`](https://github.com/ICJIA/icjia-migration-tools). Timeline reframed: "remaining front-end work is expected to take 6–8 weeks" rather than "the entire project."
- **`docs/01-EXECUTIVE-SUMMARY.md` §2 — embedded homepage render removed.** Caption paragraph and image both removed; §2 now leads with a written description of the visual direction (contemporary, dark + light mode, design-system-driven). Per user direction: the embedded render risked giving leadership the impression that a front-end was already drafted as a settled artifact; the planning repo should not telegraph that.
- **`docs/01-EXECUTIVE-SUMMARY.md` §3 — three reasons reframed.** Reasons retained but tense and emphasis adjusted: phone performance is still a current problem, the publishing tool *was* past end-of-life and has been replaced (extensive expansion — see "Added"), accessibility on the public-facing site is still held together with workarounds and is what the front-end rebuild addresses. Closing line acknowledges that two of the three reasons (CMS EOL, eventually phone performance) are on a clear path; the front-end rebuild handles what remains.
- **`docs/01-EXECUTIVE-SUMMARY.md` §6 "What the work actually is" — rewritten end-to-end.** Sub-section structure changed from 6 sub-sections (6.1–6.6) to 5 (6.1–6.5). Removed: 6.2 ("a new publishing tool is built alongside the old one") and 6.3 ("content is migrated; parity is verified") — both subsumed into a single new 6.2 "the publishing-tool upgrade was completed first (already done)" framed as past tense with link to the migration tool. The original 6.4–6.6 (visible website built section by section, testing with real people, cutover) renumbered to 6.3–6.5. Reference to "the first-draft render shown in §2" removed from the homepage bullet, since the render is no longer in §2.
- **`docs/01-EXECUTIVE-SUMMARY.md` §7 timeline — Track 2 removed.** Two-track structure replaced with a single front-end-rebuild table. Track 2 (publishing-tool upgrade) is documented as already complete in the "out of scope" callout below the table. Milestone bullets reworded to remove the "real content flows from the *updated* publishing tool" framing — the publishing tool was already updated; what flows on milestone day is the (already-migrated) Strapi 5 content into the new front end.
- **`docs/01-EXECUTIVE-SUMMARY.md` §8 leadership decisions — four → three.** Removed: "Who owns the publishing-tool upgrade?" (no longer applicable; the upgrade is done). Remaining: approval to begin the front-end rebuild, accessibility reviewer, cutover window. Renumbered.
- **`docs/01-EXECUTIVE-SUMMARY.md` §9 risks — three → two.** Removed: "The publishing-tool upgrade is on the critical path" (no longer applicable). Remaining: parallel-operation author attention, post-cutover settling-in period. Closing paragraph notes the removed risk explicitly with link to `icjia-migration-tools`.
- **`docs/01-EXECUTIVE-SUMMARY.md` §11 glossary — past-tense touch-ups.** "Migration script" entry past-tense with link to `icjia-migration-tools`. "Parity" entry past-tense with link to the audit reports. "Strapi" entry reworded to reflect the completed migration. "Content type" entry pruned (the migration-related sentence is no longer needed).
- **`docs/01-EXECUTIVE-SUMMARY.md` and `docs/06-ACCESSIBILITY-STRATEGY.md` — "residents" → "users" throughout.** 12 occurrences in the exec summary replaced with case-correct substitution; 1 occurrence in the accessibility doc reworked to "users include many with disabilities" with a "global reach" note added (accessibility legal context originally framed Illinois residents specifically).
- **`README.md` — embedded homepage render removed; status reframed.** Image and caption removed (per user direction, see §2 above). "Status" paragraph reframed: backend Strapi 3→5 migration is complete with link to `icjia-migration-tools`; planning set is for the front-end rebuild. "What this is" paragraph reworded: CMS already moved off Strapi 3, remaining work is the front-end rebuild. "Repository scope" reorganized: backend migration repo (complete, with link), Strapi 5 instance repo, forthcoming Nuxt 4 frontend repo. "UI references" section removed (the section that pointed at the mockup PNG and HTML).
- **`docs/03-STRAPI-UPGRADE-PLAN.md` — status flipped to COMPLETED.** Frontmatter status: DRAFT v0.1 → COMPLETED. Block-quote at the top of §1 notes the strategy described was followed and the migration has been executed; body retained as the record of *why* the parallel-fresh approach was chosen rather than rewritten. Tense in the §1 lead nudged to past where it makes the read cleaner ("legacy" / "was unavoidable" / "was *how*"). Structure and §2–§10 left intact.
- **`docs/04-PHASED-DELIVERABLE-PLAN.md` — Strapi track marked complete.** Frontmatter v0.2 → v0.3. Scope line: "(Nuxt track is the active work; Strapi track is COMPLETED and retained for the record)." Block-quote in "How to read this document" section notes the Strapi track is executed; cross-track gate (S7 → P4) is met. §2 third compression-factor bullet reworded to credit `icjia-migration-tools` (the actual fork that did the work) rather than `hub-migration-tools` (the upstream template).
- **`docs/07-OPEN-QUESTIONS.md` Q3 — status decided/completed.** Status flipped from "planned" to "decided / completed." Resolution paragraph from 2026-04-23 retained verbatim (the strategy decision); new "Update (2026-05-03) — completed" paragraph appended noting the migration has been executed via `icjia-migration-tools`, S7 gate is met, no remaining cross-track block. Append-only convention respected.
- **`docs/08-STRAPI-MIGRATION-RUNBOOK.md` — status flipped to COMPLETED + tool reference updated.** Frontmatter status: DRAFT v0.1 → COMPLETED ("historical record"). Tense in §1 overview adjusted to clarify this is the procedure-as-run, retained as a record. Audience reworked: "anyone reviewing how the migration was executed." Every body reference to `hub-migration-tools` replaced with `icjia-migration-tools` (the actual fork that was used). Procedural body (commands, configs, phase walk-throughs) left intact — those are the procedure that was followed and remain useful as historical record.
- **`docs/00-README.md` — document-status table refreshed.** Status column updated for documents that changed: 01 (v2.0 → v2.3), 03 (DRAFT v0.1 → COMPLETED, audience note adjusted), 04 (DRAFT v0.1 → DRAFT v0.3 with "Strapi track complete" annotation), 08 (DRAFT v0.1 → COMPLETED, audience note adjusted to "operational record of the executed migration"). Top paragraph adds the backend-migration status banner.

### Not changed

- **`docs/05-DESIGN-SYSTEM.md`** — references to `ui/ICJIA_Redesign_v2_squared.html` and the full-page PNG retained. These are technical-doc references for engineers building the design system, not embedded images in a leadership-facing surface; the user's "remove the image" direction was about the README and exec summary leading with the render, not about scrubbing every internal reference. Engineers building the design system need to see the visual reference.
- **`docs/phases/P1-design-system.md`** — same rationale as `05-DESIGN-SYSTEM.md`. P1 is the design-system implementation phase; the engineer building it needs the mockup link.
- **`docs/09-SECURITY-REQUIREMENTS.md`** — no edits in this revision. SEC items are still applicable; the backend-migration completion does not retire any of them. SEC-08 (legacy-site dependency audit during parallel operation) remains relevant to the public-site rebuild's parallel-operation window.
- **`docs/02-MASTER-DESIGN-PLAN.md` §4 (rendering and data architecture) and beyond** — no edits. The rendering pipeline, data access patterns, preview architecture, accessibility approach, success criteria all describe the front-end rebuild and are unaffected by the backend completion.
- **The historical CHANGELOG entries (0.11.0 and earlier)** — left intact per convention; the changelog is an append-only audit trail of decisions as they were made.

### Repository hygiene

- **Added** `docs/icjia-icjia-migration-tools-8a5edab282632443.txt` — directory dump of the [`ICJIA/icjia-migration-tools`](https://github.com/ICJIA/icjia-migration-tools) repository at the SHA prefix in the filename, used as the source of truth for cross-checking what the migration tool actually does (script names, configs, content-type handling, phase orchestration). Committed alongside this revision as evidence; future revisions may regenerate or remove it.
